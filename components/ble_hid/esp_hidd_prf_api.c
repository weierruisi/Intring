...
void esp_hidd_send_all_mouse_value(uint16_t conn_id, uint8_t mouse_button, int8_t mickeys_x, int8_t mickeys_y, 
                                int8_t wheel_value, int8_t AC_Pan_value)
{
    uint8_t buffer[HID_MOUSE_IN_RPT_LEN];
    ...
    buffer[1] = mickeys_x;           // X
    buffer[2] = mickeys_y;           // Y
    buffer[3] = wheel_value;           // Wheel
    buffer[4] = AC_Pan_value;           // AC Pan
